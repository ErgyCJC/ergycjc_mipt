...
Graph::Graph(int v_count) : vertices_count(v_count), edges_count(0) {
  ...
  degrees.resize(vertices_count, 0);
}
...
double Graph::GetWeightBetweenVertices(int v_index1, int v_index2) const {
  return matrix[v_index1][v_index2];
}

int Graph::GetVertexDegree(int v_index) const {
  return degrees[v_index];
}